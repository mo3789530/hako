...
- [ ] TenantとGitHub App Installation/Repositoryを対応付け、Repository Registry APIとRBACを追加する（Owner/Admin向けInstallation claim作成・一覧API、active binding/Repository schema、active InstallationのRepository一覧API、確認済みBindingに限定するRepository同期Storeを追加。GitHub callback検証・active化とWebhook dispatcher接続は未実装）。
